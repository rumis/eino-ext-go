...
// NewIndexAUTOINDEXSearchParam

// defaultSearchParam returns the default search param
func defaultSearchParam(score float64, dim float64) map[string]string {
	return map[string]string{
		"level":        "1",
		"range_filter": strconv.FormatFloat(score, 'f', -1, 64),
		"dim":          strconv.FormatFloat(dim, 'f', -1, 64),
	}
}

// defaultDocumentConverter returns the default document converter
func defaultDocumentConverter() func(ctx context.Context, doc milvusclient.ResultSet) ([]*schema.Document, error) {
	return func(ctx context.Context, doc milvusclient.ResultSet) ([]*schema.Document, error) {
		var err error
...
func loadCollection(ctx context.Context, conf *RetrieverConfig) error {
	loadState, err := conf.Client.GetLoadState(ctx, milvusclient.NewGetLoadStateOption(conf.Collection))
	if err != nil {
	...
	case entity.LoadStateNotLoad:
		index, err := conf.Client.DescribeIndex(ctx, milvusclient.NewDescribeIndexOption(conf.Collection, conf.VectorField))
		if err != nil {
	...
	case entity.LoadStateLoading:
		...
		for {
			...
			case <-ctx.Done():
				return ctx.Err()
			case <-ticker.C:
				if loadState.Progress == defaultLoadedProgress {
					return nil
				}
			}
		}
